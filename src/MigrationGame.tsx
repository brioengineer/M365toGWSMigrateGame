...
/** Single-file game for M365 → Google Workspace. Tailwind v4. */
...
function Header({score,best,mode,onChangeMode,timeLeftMs,lives,badges}:{score:number;best:number;mode:Mode;onChangeMode:(m:Mode)=>void;timeLeftMs:number|null;lives:number;badges:BadgeKey[];}){
  return (<header className="sticky top-0 z-10 backdrop-blur bg-white/70 dark:bg-gray-900/70 border-b border-gray-200 dark:border-gray-800">
...
function MatchMode({onResult,onRoundEnd}:{onResult:(d:number,c:boolean,why?:string)=>void;onRoundEnd:(s:{total:number;correct:number;avgMs:number})=>void;}){
  ...
  return (<div className="grid md:grid-cols-2 gap-4">
    <Card><h2 className="text-lg font-semibold mb-2">M365 Apps</h2><ul className="grid gap-2">{left.map(l=>(
      <li key={l}><button onClick={()=>handlePick("L",l)} disabled={matched[l]!==undefined}
        className={cls("w-full text-left px-4 py-3 rounded-xl border", selLeft===l?"border-blue-600 ring-2 ring-blue-600/30":"border-gray-300 dark:border-gray-700", matched[l]!==undefined?"opacity-50 cursor-not-allowed":"hover:bg-gray-50 dark:hover:bg-gray-800")}>{l}</button></li>
    ))}</ul></Card>
    <Card><h2 className="text-lg font-semibold mb-2">Google Workspace</h2><ul className="grid gap-2">{right.map(r=>(
      <li key={r}><button onClick={()=>handlePick("R",r)} className="w-full text-left px-4 py-3 rounded-xl border border-gray-300 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800">{r}</button></li>
    ))}</ul></Card>
...
function QuizMode({onResult,onRoundEnd}:{onResult:(d:number,c:boolean,why?:string)=>void;onRoundEnd:(s:{total:number;correct:number;avgMs:number})=>void;}){
  ...
  const choose=(c:string)=>{ if(lock) return; setSelected(c); setLock(true);
    const ok=c===q.answer; const dt=Date.now()-t0; times.current.push(dt);
  ...
  return (<div className="grid gap-4">
    <Card>
      <div className="flex items-center justify-between mb-2"><h2 className="text-lg font-semibold">Question {idx+1} / {questions.length}</h2><div className="text-sm">⏱ {formatTime(timeLeft)}</div></div>
      <p className="text-base mb-4 font-medium">{q.q}</p>
      <ul className="grid gap-2">{options.map(c=>{ const isSel=selected===c; const isCorrect=lock && c===q.answer;
        const style=lock?(isCorrect?"border-green-600 bg-green-50 dark:bg-green-900/20": isSel?"border-red-600 bg-red-50 dark:bg-red-900/20":"opacity-80"):"hover:bg-gray-50 dark:hover:bg-gray-800";
        return (<li key={c}><button onClick={()=>choose(c)} disabled={lock} className={cls("w-full text-left px-4 py-3 rounded-xl border",style)}>{c}</button></li>); })}
      </ul>
      <div className="mt-3 flex items-center justify-between">
        <div className="text-sm opacity-80" aria-live="polite">{lock && (selected===q.answer?"✅ Correct": selected? "❌ Incorrect":"⌛ Time's up")}</div>
...
export default function MigrationGame(){
  ...
  const onResult=(d:number,ok:boolean,why?:string)=>{ setScore(s=>s+d); if(!ok) setLives(l=>Math.max(0,l-1)); console.debug(ok?"OK:":"NO:",why??""); };
  const onRoundEnd=(stats:{total:number;correct:number;avgMs:number})=>{
    const accuracy=stats.total?Math.round((stats.correct/stats.total)*100):0;
    setModeScores(ms=>({...ms,[mode]:accuracy}));
    setBadges(prev=>{ const next=new Set(prev);
