  ...
  return (
    <button
      ...
      className={cls(
        ...
        active ? "bg-blue-600 text-white border-blue-600" : "bg-white dark:bg-gray-900 border-gray-300 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800"
      )}
      aria-pressed={active}
    >
      {label}
...
function BadgeBar({ badges }: { badges: BadgeKey[] }) {
  if (!badges.length) return null;
  const desc: Record<BadgeKey, string> = {
    Speedster: "Answered fast",
    PerfectRound: "No mistakes",
    MigrationPro: "80%+ in all modes",
    ComebackKid: "Bounced back",
    MatchMaster: "Match mode star",
    QuizWhiz: "Quiz champ",
    ScenarioSage: "Scenario solver",
  };
  return (
    <div className="flex flex-wrap items-center gap-2 py-2">
      {badges.map((b) => (
        <span
          key={b}
          className="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs bg-amber-100 text-amber-900 dark:bg-amber-800/40 dark:text-amber-100 border border-amber-300/60"
        >
          <span>🏅</span>
          <span>{b}</span>
          <span className="opacity-70">— {desc[b]}</span>
        </span>
      ))}
    </div>
  );
...
function Card({ children, className }: { children: React.ReactNode; className?: string }) {
  return <div className={cls("rounded-2xl shadow-sm border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 p-4", className)}>{children}</div>;
}
...
function Header(props: {
  ...
  const { score, best, mode, onChangeMode, timeLeftMs, lives, badges } = props;
  return (
...
function QuizMode(props: {
  ...
  const next = () => {
    if (idx + 1 >= questions.length) {
      ...
      setTimeout(() => { times.current = []; setQuestions(shuffle([...SHORTCUT_QA, ...TERMINOLOGY_QA]).slice(0, 8)); setIdx(0); setSelected(null); setLock(false); setT0(Date.now()); setTimeLeft(20000); }, 600);
      return;
    }
